...
export default function FullAdminDashboard() {
    ...
    const handleUserSelect = async (user) => {
        ...
        try {
            ...
            console.error("Post count failed");
        } finally {
    ...
    const sendSinglePush = async () => {
        if (!pushMessage.title || !pushMessage.body) return toast.warning("Enter message details");
        setSendingPush(true);
        try {
            const res = await fetch('/api/admin/notifications', {
                method: 'POST',
            ...
            if (res.ok) {
                toast.success("Signal Sent to Operator");
                setPushMessage({ title: "", body: "" });
            }
        } catch (err) { toast.error("Signal Failed"); }
        finally { setSendingPush(false); }
    };

    const sendBulkPush = async () => {
        const msg = prompt(`Send notification to ${dormantCount} users offline for 30+ days. Enter message:`);
        if (!msg) return;
        setSendingPush(true);
        try {
            const res = await fetch('/api/admin/notifications', {
            ...
            if (res.ok) toast.success("Bulk Signal Broadcasted!");
    ...
    if (initialLoading) return (
        <div className="flex h-screen items-center justify-center bg-white dark:bg-[#0a0a0a]">
            <div className="flex flex-col items-center">
                ...
                <p className="mt-4 text-[10px] font-black uppercase tracking-widest text-gray-400 animate-pulse">Engaging Systems...</p>
            </div>
        </div>
    );

    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-8 dark:bg-[#0a0a0a] text-gray-900 dark:text-gray-100 transition-colors">
            <div className="mx-auto max-w-7xl">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            ...
                            <span className="bg-orange-500 text-white p-1 rounded-lg group-hover:bg-white group-hover:text-orange-500">🚀</span>
                        </button>
                        ...
                        <div className="flex bg-white dark:bg-gray-800 p-1 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                            {["24h", "7days", "30days"].map((r) => (
                                <button 
                                    key={r}
                                    onClick={() => setRange(r)}
                                    className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase transition-all ${range === r ? "bg-blue-600 text-white shadow-lg" : "text-gray-400 hover:text-blue-500"}`}
                                >
                                    {r === '24h' ? 'Live' : r === '7days' ? 'Weekly' : 'Monthly'}
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-200 dark:border-gray-700 relative overflow-hidden">
                        <h3 className="font-black text-[10px] uppercase tracking-widest mb-10 text-gray-400 flex items-center gap-2">
                            ...
                            {stats?.dailyActivity?.map((data, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex-1 flex flex-col items-center">
                                        <span className="text-[10px] font-black text-blue-600 mb-1">{data.count}</span>
                                        <div className={`w-full max-w-[24px] rounded-t-lg transition-all duration-1000 ${data.count > 0 ? 'bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.2)]' : 'bg-gray-100 dark:bg-gray-700'}`} style={{ height: `${Math.max(barHeight, 5)}px` }}></div>
                                        <p className="text-[8px] mt-3 text-gray-400 font-black uppercase text-center tracking-tighter">{displayDate}</p>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-200 dark:border-gray-700">
                        <h3 className="font-black text-[10px] uppercase tracking-widest mb-6 text-gray-400 italic">Origin Pulse</h3>
                ...
                {/* --- USER TABLE --- */}
                <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-xl mb-6">
                    <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <select value={selectedCountry} onChange={(e) => { setSelectedCountry(e.target.value); setPage(1); }} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-2 px-4 rounded-xl text-[9px] font-black uppercase outline-none focus:ring-2 ring-blue-500">
                            ...
                            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                                {tableLoading ? (
                                    <tr>
                                        <td colSpan="4" className="p-20 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="font-black text-gray-300 text-[9px] uppercase tracking-widest">Querying DB...</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : userList.map((u) => (
                                    <tr 
                                        ...
                                        className={`cursor-pointer transition-all ${selectedUser?._id === u._id ? 'bg-blue-600/10 border-l-4 border-blue-600' : 'hover:bg-gray-50 dark:hover:bg-white/5 border-l-4 border-transparent'}`}
                                    >
                                        <td className="px-8 py-4">
                                            <div className="flex items-center gap-4">
                                                <img 
                                                    src={u.profilePic?.url || u.image || u.avatar || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'} 
                                                    className="h-10 w-10 rounded-2xl object-cover border border-gray-200 dark:border-gray-700 bg-gray-100" 
                                                    alt="pfp" 
                                                />
                                                <div>
                                        ...
                                        <td className="px-8 py-4 text-[10px] font-bold text-gray-400 uppercase">
                                            {new Date(u.lastActive).toLocaleString('en-NG', { dateStyle: 'short', timeStyle: 'short' })}
                                        </td>
                                        <td className="px-8 py-4">
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-900/30 flex justify-between items-center px-8 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div className="flex gap-2">
                            <button disabled={page === 1 || tableLoading} onClick={() => { setPage(p => p - 1); setSelectedUser(null); }} className="px-5 py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 disabled:opacity-30 font-black text-[9px] uppercase hover:shadow-md transition-all active:scale-95">PREV</button>
                            <button disabled={page === totalPages || tableLoading} onClick={() => { setPage(p => p + 1); setSelectedUser(null); }} className="px-5 py-2 rounded-xl bg-blue-600 text-white font-black text-[9px] uppercase hover:bg-blue-700 shadow-lg shadow-blue-500/20 active:scale-95">NEXT</button>
                        </div>
                    </div>
                </div>
                ...
                {selectedUser && (
                    <div className="bg-white dark:bg-gray-800 rounded-3xl border-2 border-blue-600/30 p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500 mb-20 relative overflow-hidden">
                        ...
                        <div className="flex flex-col lg:flex-row gap-10">
                            ...
                            <div className="flex flex-col items-center lg:items-start shrink-0">
                                <img 
                                    ...
                                    className="h-36 w-36 rounded-[2.5rem] object-cover border-4 border-white dark:border-gray-700 shadow-2xl mb-6 bg-gray-100" 
                                    alt="Avatar" 
                                />
                                <Link 
                            ...
                            <div className="flex-1">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    <div className="lg:col-span-2">
                                        <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1 italic">Username & Signature</label>
                                        <h2 className="text-3xl font-black italic tracking-tighter uppercase mb-1">{selectedUser.username || "ANONYMOUS OPERATOR"}</h2>
                                        <p className="text-[10px] font-mono text-blue-500 break-all bg-blue-500/5 p-2 rounded-lg inline-block">{selectedUser._id}</p>
                                    </div>

                                    <div className="bg-blue-600/5 p-4 rounded-3xl border border-blue-600/10 text-center flex flex-col justify-center">
                                        <label className="text-[9px] font-black text-gray-400 uppercase block mb-1">Transmission Count</label>
                                        {userMetaLoading ? (
                                            <div className="h-8 w-12 bg-gray-200 dark:bg-gray-700 animate-pulse rounded mx-auto"></div>
                                        ) : (
                                            <p className="text-4xl font-black text-blue-600">{selectedUser.postCount} <span className="text-[10px] tracking-widest italic opacity-50">POSTS</span></p>
                                        )}
                                    </div>

                                    {/* SIGNAL COMPOSER SECTION */}
                                    <div className="lg:col-span-3 bg-gray-50 dark:bg-gray-900/50 p-6 rounded-[2rem] border border-gray-200 dark:border-gray-700">
                                        <div className="flex items-center gap-3 mb-6">
                                            <span className="p-2 bg-blue-600 rounded-lg text-white text-xs">🔔</span>
                                            <h4 className="font-black text-xs uppercase tracking-[0.2em]">Signal Composer</h4>
                                        </div>
                                        
                                        {!selectedUser.pushToken ? (
                                            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-red-500 font-black text-[10px] uppercase text-center">
                                                User has no active Push Token. Signal transmission impossible.
                                            </div>
                                        ) : (
                                            <div className="space-y-4">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <input 
                                                        ...
                                                        className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 text-[11px] font-black uppercase outline-none focus:ring-2 ring-blue-500 transition-all"
                                                    />
                                                    <input 
                                                        type="text" 
                                                        ...
                                                        className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 text-[11px] font-black outline-none focus:ring-2 ring-blue-500 transition-all"
                                                    />
                                                </div>
                                                <button 
                                                    onClick={sendSinglePush}
                                                    disabled={sendingPush}
                                                    className="w-full bg-gray-900 dark:bg-white text-white dark:text-gray-900 py-4 rounded-xl font-black text-[10px] uppercase tracking-[0.3em] hover:opacity-90 disabled:opacity-50 transition-all flex items-center justify-center gap-3"
                                                >
                                    ...
                                    <div className="md:col-span-2 lg:col-span-1">
                                        <label className="text-[9px] font-black text-gray-400 uppercase block mb-1">Signal Token (Push)</label>
                                        <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700 max-h-20 overflow-y-auto">
                                            <p className="text-[9px] font-mono text-gray-400 break-all leading-relaxed">
                                                {selectedUser.pushToken || "COMMUNICATION_LINK_OFFLINE"}
                                            </p>
                                        </div>
                                    </div>
                                </div>
...
function MetricCard({ title, value, color, loading }) {
    return (
        <div className="p-6 rounded-3xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1 italic">{title}</p>
            {loading ? (
