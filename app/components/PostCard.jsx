...
export default function PostCard({ post, posts, setPosts, isFeed, hideComments = false, hideMedia, className }) {
	...
	const handleShare = async () => {
		try {
			...
			updatePostLocally({ ...post, shares: totalShares + 1 });
	...
	useEffect(() => {
		...
		if (!window.__tiktokScriptLoaded) {
			...
			// If script already exists, reprocess TikTok embeds
			if (window.tiktokEmbedder?.processEmbeds) {
				window.tiktokEmbedder.processEmbeds();
			} else {
				// fallback if TikTok namespace isn't exposed yet
				setTimeout(() => {
			...
			<div className={`bg-white dark:bg-gray-800 shadow-md rounded-md p-4 mb-6 relative overflow-hidden ${className}`}>
				...
				<div className="flex justify-between items-center mb-1">
					<Link href={`/author/${post.authorId}`} className="flex items-center space-x-2 hover:underline">
						...
						<span className="font-semibold text-2xl underline capitalize">{author.name || "Unknown"}</span>
					</Link>
