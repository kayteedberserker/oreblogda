...
export async function POST(req) {
    try {
        ...
        // 🧠 CREATE CANONICAL CORE
        const cleanNewName = normalizedUsername.toUpperCase().replace(/[^A-Z0-9]/g, "");

        // ============================================================================
        // 🛡️ IDENTITY COLLISION & LOCK CHECKS
        // ============================================================================

        // 🛡️ 1. CANONICAL COLLISION CHECK (Premium Players with Active Locks)
        const activeLock = await MobileUser.findOne({
        ...
        const nameExists = await MobileUser.findOne({
            username: { $regex: new RegExp(`^${safeRegexName}$`, "i") }
        }).select("_id");

        if (nameExists) {
            return NextResponse.json({
                message: "This exact username is already claimed by an active operator."
            }, { status: 409 });
        }
        // 🛡️ END NAME CHECKS
        ...
        const cleanName = normalizedUsername.toUpperCase().replace(/\s+/g, "_");
        ...
        const user = await MobileUser.create({
            ...
            canonicalUsername: cleanNewName, // ⚡️ CRITICAL: Saved on creation!
            pushToken,
            ...
            preferences,
            character: character || {
                base: { gender: 'male', skinTone: 'medium', name: normalizedUsername },
