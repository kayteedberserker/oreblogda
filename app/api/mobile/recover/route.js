    ...
    try {
        ...
        if (requiresPinAuth && !isTrustedDevice) {
            ...
            if (!isMatch) {
                ...
                // ⚡️ FIXED: Surgical update to prevent corrupted map data from crashing the DB save
        ...
        // ⚡️ FIXED: Surgical update to bypass full document validation
        // This allows users with corrupted legacy affinityScores to still log in successfully
        await MobileUser.updateOne(
            { _id: user._id },
            {
                $set: {
                    ...
                    activeSessionDeviceId: user.activeSessionDeviceId,
                    country: user.country,
                    hasLoggedOut: user.hasLoggedOut,
