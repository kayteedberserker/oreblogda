...
const SingleComment = ({ comment, onOpenDiscussion }) => {
    ...
    return (
        ...
        >
            ...
            {comment.isHidden ? (
                <p className="text-xs text-red-500/80 dark:text-red-400/80 font-bold italic leading-5 mt-1">
                    This signal is hidden due to community guidelines not followed.
                </p>
            ) : (
                <>
                    {comment.imageUrl && (
                        <div className="mt-2 mb-2 relative max-w-sm rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-900">
            ...
            <div className="flex items-center mt-2 gap-4">
                <span className="text-gray-400 text-[10px] font-bold flex items-center gap-1">
                    ...
                    {comment.isEdited && <span className="text-blue-400/80">(edited)</span>}
                </span>
                <button
            ...
            {hasReplies && previewReply && (
                <div className="mt-3 opacity-60 bg-gray-50 dark:bg-white/5 p-2 rounded-lg border-l border-gray-300 dark:border-gray-700">
                    ...
                    <p className="text-[11px] text-gray-500 font-bold truncate">
                        {previewReply.isHidden ? "Hidden signal" : previewReply.text || (previewReply.imageUrl ? "[Image]" : "")}
                    </p>
                </div>
            )}
        </motion.div>
    ...
    return (
        ...
        >
            ...
            {reply.isHidden ? (
                ...
                <>
                    {reply.replyToName && reply.replyToText && (
                        <div className="mt-2 mb-1 pl-3 border-l-2 border-blue-400/50 bg-blue-50/50 dark:bg-blue-900/10 rounded-r py-1 text-[10px]">
                            <span className="font-bold text-blue-600 dark:text-blue-400">@{reply.replyToName}</span>
                            <p className="text-gray-500 dark:text-gray-400 truncate opacity-80">{reply.replyToText}</p>
                        </div>
                    )}
                    {reply.imageUrl && (
                        <div className="mt-2 mb-2 relative max-w-sm rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-900">
                            <img src={reply.imageUrl} alt="Reply Attachment" className="w-full h-auto object-cover" loading="lazy" />
                        </div>
                    )}
                    <p className="text-xs text-gray-600 dark:text-gray-300 font-bold leading-5 mt-1">
                        {reply.text}
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase mt-2 flex gap-1 items-center">
                {new Date(reply.date).toLocaleTimeString()}
                {reply.isEdited && <span className="text-blue-400/80 normal-case">(edited)</span>}
            </span>
        </motion.div>
    );
};
                ...
                <div className="bg-white dark:bg-[#0a0a0a] border-b border-gray-100 dark:border-gray-800 z-10 shrink-0">
                    ...
                    <div className="bg-blue-50/50 dark:bg-blue-900/10 px-6 py-4 border-y border-blue-100 dark:border-blue-900/30">
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400 font-bold leading-5 line-clamp-3">
                            {comment.isHidden ? <span className="text-red-500/80 italic">This signal is hidden.</span> : comment.text}
                        </p>
                    </div>
                </div>
    ...
    const handlePostComment = async (parentId = null, replyContent = null) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (!parentId) {
                    // Clear input and instantly append to top level state
                    setText("");
                    setPagedComments(prev => [newComment, ...prev]);
                } else {
                    // Instantly inject reply into active discussion view
                    ...
                    // Update background state so reply count stays accurate
                ...
                // Force SWR to background sync the latest DB data safely
                await mutate();
            }
        } catch (err) {
