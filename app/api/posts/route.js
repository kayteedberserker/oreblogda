...
const FEED_ALGORITHM_VERSION = "personalized-feed-v4-fresh-stable";
...
function getPostIdString(value) {
    if (!value) {
        return "";
    }
...
function deduplicatePostsById(posts) {
    const seenIds = new Set();
    const uniquePosts = [];

    for (const post of Array.isArray(posts) ? posts : []) {
        const postId = getPostIdString(post);
...
function deduplicateSessionEntries(entries) {
    const seenIds = new Set();
    const uniqueEntries = [];

    for (const entry of Array.isArray(entries) ? entries : []) {
        const postId = entry?.postId?.toString?.();
...
function getEffectivePostTimestamp(post) {
    ...
    const timestamp = new Date(effectiveValue || 0).getTime();
...
function prioritizeFreshAndBoostedPosts(
    rankedPosts,
    pageSize,
    nowMs = Date.now()
) {
    const uniquePosts = deduplicatePostsById(rankedPosts);
    ...
    const recentBoosted = uniquePosts
        .filter(post =>
            Boolean(post.isActiveBoost)
            && getEffectivePostTimestamp(post) >= freshCutoffMs
        )
        .sort((a, b) =>
            getEffectivePostTimestamp(b)
            - getEffectivePostTimestamp(a)
        );
    ...
    const olderBoosted = uniquePosts
        .filter(post =>
            Boolean(post.isActiveBoost)
            && getEffectivePostTimestamp(post) < freshCutoffMs
        )
    ...
    const freshTarget = Math.min(
        freshPosts.length,
        Math.max(
            5,
            Math.ceil(topWindowSize * 0.6)
        )
    );

    const priorityPosts = deduplicatePostsById([
        ...recentBoosted.slice(0, 2),
        ...freshPosts.slice(0, Math.min(2, freshTarget)),
        ...olderBoosted.slice(0, 1),
        ...freshPosts.slice(2, freshTarget)
    ]);
    ...
    const priorityIds = new Set(
        priorityPosts.map(post => getPostIdString(post))
    );

    const remainingPosts = uniquePosts.filter(
        post => !priorityIds.has(getPostIdString(post))
    );
    ...
    while (
        topWindow.length < topWindowSize
        && (
            priorityIndex < priorityPosts.length
            || remainingIndex < remainingPosts.length
        )
    ) {
        for (
            let count = 0;
            ...
            topWindow.push(priorityPosts[priorityIndex++]);
        }
        ...
        if (
            ...
            topWindow.push(remainingPosts[remainingIndex++]);
        }
    }
    ...
    const usedIds = new Set(
        topWindow.map(post => getPostIdString(post))
    );

    return [
        ...topWindow,
        ...uniquePosts.filter(
            post => !usedIds.has(getPostIdString(post))
        )
    ];
}
...
function getExploreBucket(postId, candidateMap) {
    ...
    const hasPrioritySource = sources.some(source =>
        ["boosted", "clan", "author", "interest", "trending"].includes(source.type)
    );
    ...
    try {
        ...
        if (
            ...
            return NextResponse.json(
                {
                    code: "FEED_CURSOR_REQUIRED",
                    message:
                        "A cursor is required when continuing a feed session."
                },
                { status: 400 }
            );
        }
            ...
            return NextResponse.json(
                {
                    ...
                    message:
                        "The supplied feed cursor is invalid."
                },
                { status: 400 }
            );
        }
        ...
        const isSessionContinuation = Boolean(
        ...
        const [userProfile, follows, memberships] = await Promise.all([
            deviceId && !isSessionContinuation
            ...
            viewerId && !isSessionContinuation
                ? Clan.find({
                    $or: [
                        { leader: viewerId },
                        { viceLeader: viewerId },
        ...
        if (userProfile) {
            ...
            // Protect the feed from learned weights starving fresh content or
            // allowing archival discovery to dominate the candidate set.
            dynamicWeights.fresh = Math.min(
                0.40,
                Math.max(
                    0.18,
                    Number(dynamicWeights.fresh) || 0.25
                )
            );

            dynamicWeights.explore = Math.min(
                0.18,
                Math.max(
                    0.08,
                    Number(dynamicWeights.explore) || 0.15
                )
            );

            dynamicWeights.trending = Math.min(
                0.30,
                Math.max(
                    0.10,
                    Number(dynamicWeights.trending) || 0.15
                )
            );
        ...
        if (
            ...
            const entries =
                deduplicateSessionEntries(rawEntries);

            if (entries.length !== rawEntries.length) {
                console.warn(
                    "Duplicate feed-session entries ignored:",
            ...
            const expectedCursor = Math.max(
                0,
                Number(existingSession.highestServedOffset) || 0
            );
                ...
                return NextResponse.json(
                    {
                        code:
                            requestedCursor < expectedCursor
                                ? "FEED_CURSOR_ALREADY_SERVED"
                                : "FEED_CURSOR_GAP",
                        message:
                            requestedCursor < expectedCursor
                                ? "That feed page was already served. Continue from the expected cursor."
                                : "The feed cursor skipped part of this session.",
                        expectedCursor,
            ...
            if (requestedCursor > entries.length) {
                return NextResponse.json(
                    {
                        code: "FEED_CURSOR_OUT_OF_RANGE",
                        message:
                            "The feed cursor is beyond this session snapshot.",
            ...
            const safeCursor = requestedCursor;
            ...
            if (!sessionAdvanceResult.matchedCount) {
                return NextResponse.json(
                    {
                        code: "FEED_CURSOR_CONFLICT",
                        message:
                            "Another request already advanced this feed session.",
                        expectedCursor:
                            responseNextCursor
                    },
                    { status: 409 }
        ...
        if (sessionPageRankedRows) {
            ...
            const POOL_CONFIG = {
                freshPool: Math.max(
                    Math.floor(poolBudget * dynamicWeights.fresh),
                ...
                trendingPool: Math.max(
                    Math.floor(poolBudget * dynamicWeights.trending),
                    Math.min(60, limit * 2)
                ),
                explorePool: Math.min(
                    Math.floor(poolBudget * dynamicWeights.explore),
                    Math.max(30, limit * 3)
                )
            };
            ...
            const [
                ...
                // Active paid boosts must never depend on learned pool weights.
                Post.find({
                    ...basePoolQuery,
                    boostedUntil: { $gt: now }
                })
                    ...
                    .limit(60)
                    .select("_id boostedUntil createdAt")
                    .lean(),
        ...
        if (sessionPageRankedRows) {
            ...
            const CONFIG = {
                ...
                // Freshness now fades gradually instead of disappearing after 3 hours.
                freshnessBoost: 45,
                freshnessDecayHours: 14,

                // Older posts still have a chance, but cannot dominate a slow feed.
                normalHalfLifeHours: 18,
                clanHalfLifeHours: 36,
                exploreHalfLifeHours: 72,

                normalGravityPower: 1.25,
                clanGravityPower: 1.15,
                exploreGravityPower: 1.15,
            ...
            // Keep a single copy of each post through every ordering stage.
            rankedPosts = deduplicatePostsById(rankedPosts);

            // Diversity runs before the source-aware composition.
            if (rankedPosts.length > 0) {
            ...
            // Give recent and actively boosted posts protected visibility near the
            // top without discarding the underlying personalized ranking.
            rankedPosts = prioritizeFreshAndBoostedPosts(
                rankedPosts,
                limit,
                now.getTime()
            );

            const mixedPosts = deduplicatePostsById(
                buildBucketAwareExploreFeed(
                    rankedPosts,
                    candidateMap,
                    limit,
                    0.15
            ...
            if (isPersonalizedFeedRequest) {
                ...
                const entries = deduplicateSessionEntries(
                    snapshotRows.map(
                        post => {
                            const postId =
                                post._id.toString();
                ...
                // Do not delete other unexpired snapshots here. A duplicate initial
                // request or a manual refresh may still have pagination in flight for
                // the previous session. The TTL index removes expired sessions safely.
