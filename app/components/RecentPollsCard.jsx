...
export default function RecentPollsCard() {
    ...
    const { data, error, isLoading } = useSWR(
        ...
        fetcher,
        {
            ...
            revalidateOnFocus: false, // Prevents aggressive refreshing when switching tabs on phone
            revalidateIfStale: false, // Prevents refetching when navigating back to the page (remounting)
            shouldRetryOnError: false
        }
    );

    // Hardened data extraction
    const polls = data?.posts ? (Array.isArray(data.posts) ? data.posts : []) : (Array.isArray(data) ? data : []);
    ...
    if (error) return (
        <div className="p-6 bg-red-500/5 border border-red-500/20 rounded-3xl text-center">
            ...
            <p className="text-[8px] text-gray-500 mt-2 font-mono">{error.message}</p>
        </div>
    );

    // Hydration safety: Return null or placeholder if not mounted
                ...
                {polls.length > 0 ? (
                    <ul className="space-y-8 pb-4">
                        {polls.map((poll, index) => {
                            // Extra safety: Check if poll and poll._id exist before rendering
                            if (!poll?._id) return null;
                                ...
                                >
                                    ...
                                    <p className="text-xs font-bold leading-relaxed text-gray-800 dark:text-gray-200 mb-4 px-1">
                                        ...
                                        {poll.message?.length > 60
                                            ...
                                            : poll.message || "No Directive Message"}
                                    </p>
                                    ...
                                    <div className="bg-gray-50/50 dark:bg-gray-950/50 rounded-2xl border border-gray-100 dark:border-gray-800 p-1">
                                        {poll.poll ? (
                                            ...
                                            <p className="text-[8px] p-4 text-center opacity-50">Data_Corruption: Poll_Missing</p>
                                        )}
                                    </div>

                                    <div className="mt-3 flex justify-end">
                                        <Link
                                            href={`/post/${poll.slug || poll._id}`}
